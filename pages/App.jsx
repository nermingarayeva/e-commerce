...
function App() {
  ...
  const [orderedItems, setOrderedItems] = useState([]); // Track ordered items
  const [priceFilter, setPriceFilter] = useState({ min: 0, max: Infinity });
  const [categoryFilter, setCategoryFilter] = useState("all");
  const [colorFilter, setColorFilter] = useState(""); // New color filter state
  const [sizeFilter, setSizeFilter] = useState(""); // New size filter state
  ...
  useEffect(() => {
    ...
    setOrderedItems(storedBasket); // Set orderedItems state from localStorage
  }, []);
  ...
  // Add or remove product from basket
  const addToBasket = (product) => {
    const storedBasket = JSON.parse(localStorage.getItem("basket")) || [];
    const existingProduct = storedBasket.find((item) => item.id === product.id);

    if (existingProduct) {
      // If the product is already in the basket, remove it
      const updatedBasket = storedBasket.filter(
        (item) => item.id !== product.id
      );
      localStorage.setItem("basket", JSON.stringify(updatedBasket));
      setOrderedItems(updatedBasket); // Update the orderedItems state
    } else {
      ...
      setOrderedItems(storedBasket); // Update the orderedItems state
    }
  };
